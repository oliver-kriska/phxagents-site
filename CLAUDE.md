...
- Narrative pages are **`.astro` files**, not MDX: `index.astro`, `install.astro`, `iron-laws.astro`, `changelog.astro`, `catalog.astro`, `tidewave-mcp.astro`, `research/index.astro`, `research/elixir-retrieval-gap.astro`, `404.astro`. (`iron-laws.astro` and `changelog.astro` use `marked` to render some inline markdown.)
